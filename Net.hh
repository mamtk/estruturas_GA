...
public:
	...
	std::vector<std::int_fast32_t> getBlockedStations();
	std::vector<std::int_fast32_t> getActiveStations();

	std::vector<std::int_fast32_t> getTerminatedStations();
	std::vector<std::int_fast32_t> getAliveStations();

	bool getBlockedID(std::int_fast32_t id) { for(auto& i : _mClients) { if(i.getID() == id) return i.getBlocked(); } }
	bool getTerminatedID(std::int_fast32_t id) {
		for(auto& i : _mClients) { if(i.getTerminated() == id) return i.getBlocked(); }
	}
	std::string getUserName(std::int_fast32_t id) {
	...
	std::int_fast32_t addStation(std::string uname);
